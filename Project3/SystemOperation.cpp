    ...
    for(auto mdc : processOperations) {
        if(mdc.getCode() == 'P' && mdc.getDescriptor() == "run") {
            ...
            if(mdc.getDescriptor() == "hard drive") {
                ...
                this->processIOOperation(mdc, cf, this->harddriveSemaphore, this->harddriveLock, this->harddriveInCount, p.getProcessCount(), cf->getHCT(), "hard drive input on HDD", "hard drive input");
                p.setProcessState(Process::ProcessState::READY);
            ...
            if(mdc.getDescriptor() == "hard drive") {
                ...
                this->processIOOperation(mdc, cf, this->harddriveSemaphore, this->harddriveLock, this->harddriveOutCount, p.getProcessCount(), cf->getHCT(), "hard drive output on HDD", "hard drive output");
                p.setProcessState(Process::ProcessState::READY);
                ...
                this->processIOOperation(mdc, cf, this->projectorSemaphore, this->projectorLock, this->projectorCount, p.getProcessCount(), cf->getProCT(),  "projector output on PROJ", "projector output");
                p.setProcessState(Process::ProcessState::READY);
            } else {
